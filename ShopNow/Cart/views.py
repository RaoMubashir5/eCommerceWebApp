...
class AddcartView(APIView):
    ...
    def post(self,request):
         # from the request data extract all needed attributes.
        cart_product_id = request.data.get('cart_product')
        cart_id = request.data.get('cart')
        product_quantity = request.data.get('product_quantity', 1)

        # Get the cart and product to check either it is already in cart or not.
    ...
    def delete(self,request,product_id):
        #product name would be send as pk 
            owner_cart_user=request.user
